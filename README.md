...
### 📋 健身记录
主展示区按日期分组展示训练记录，支持搜索、编辑和删除。每个动作单独成条，日期标题后会显示当天最新体重。移动端支持点按条目编辑、长按删除，并会在删除前弹出确认。

### 🔥 肌肉热力图
根据记录中选择的动作，自动统计肌群出现次数，并在正面 / 背面视图中展示热力强度。热力图支持时间范围切换，包含“全部”。移动端会将热力区收纳成细横条，展开后可切换正面、背面和体重面板。

### 📈 体重变化
右侧热力图下方会同步展示体重折线图，时间范围与肌肉热力图一致。

### 📱 移动端优化
移动端主操作区使用单列布局，动作库和新增记录按钮并排显示；添加记录弹窗的肌群标签支持横向切换；热力图展开后会撑满当前可用页面区域，细横条保持可见。
...
5. 桌面端右侧热力图侧边栏可固定、收起并手动拖拽宽度；移动端点按热力区细横条展开，使用下拉框切换时间范围
